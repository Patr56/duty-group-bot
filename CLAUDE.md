...
- Долгосрочно каждый дежурит ровно `total_picks · k / N` раз; разница между max и min count в любой момент ≤ 1, **в том числе при `/reg` mid-run**.
- Новый `/reg`-нутый член стартует со `servedCount = min(существующих счётчиков)` (или 0 в пустом чате). Это ключевая вещь: если бы новичок стартовал с 0, в чате со старыми участниками с большими счётчиками он бы дежурил подряд `max−0` дней пока не догонит — что воспринимается как «новенький дежурит вообще без перерыва». При init=min он сразу встаёт в очередь наравне с самым свежим. Реализовано в `Storage.addMember` (одна транзакция: `INSERT ... SELECT COALESCE(MIN(served_count), 0u)`).
